...
/* Starting Drop Frequency of Tetrominos */
#define DEFAULT_PLAY_AREA_GRAVITY 14 /* Default = 14 gametimer ticks (70hz/10 is 1 second) */

#define LINE_CLEAR_COUNT_LEVEL_INCREASE 10 /* Default = 10 */
 ...
 * or a stationary block, in the play area (x , y) grid 
 * system.
 */
typedef struct block{
    ...
    int is_occupied, is_tetromino_piece;
...
/********** Game Model Object **********
 * 
 * Overarching Game Object Wrapper
 */
typedef struct game_model {
    Tetromino t;
    Tetromino next;     /* Next Tetromino to load into game */
    Tetromino temp;     /* Temp Tetromino for potential copying */
    Play_area p;
    Score_display sd;
    Top_score_label tsl;
    ...
    int totalClearedLinesCounter; /* Keeping track of cleared lines for level incrementing */
    int scoreIncrementCounter; /* Keeps track of how many cleared lines occur to increment score by 'n' amount*/
    int isGameOver;
    int horizontalShift; /* -1 (Left), 0 (No Move), 1 (Right) */
    int verticalShift;   /* 0 (No Move), 1 (Down) */
    int rotate;          /* -1 (Counter-clockwise), 1 (Clockwise) */
    int gameTime;        /* Timer For Rendering */
...
char randomize(Tetromino *t); /* Tested: Yes */

/* Sets a tetromino values to be cleared (set to zero) for all its block coordinates. */
void clearTetrominoValues(Tetromino *t); /* Tested: Yes */

/* Copies the tetromino blocks to their respective coordinates to the play area */
void copyTetrominotoToPlayArea(Tetromino *t, Play_area *p); /* Tested: Yes */

/* Clears the tetromino blocks at their respective coordinates to the play area. */
void clearTetrominoFromPlayArea(Tetromino *t, Play_area *p); /* Tested: Yes */

/* Sets the tetromino coordinates as ONLY occupied, sets is_tetromino flag to false */
...
int shiftTetrominoDown(Tetromino *t, Play_area *p); /* Tested: Yes */

/* Clears the current tetromino location, shifts left if valid, then copies back to the arena */
int shiftTetrominoLeft(Tetromino *t, Play_area *p); /* Tested: Yes */

/* Clears the current tetromino location, shifts right if valid, then copies back to the arena */
int shiftTetrominoRight(Tetromino *t, Play_area *p); /* Tested: Yes */

/* Rotates a tetromino on it's origin point clockwise 90 degrees if the desination coordinates are valid */
int rotateTetrominoCWise(Tetromino *t, Play_area *p); /* Tested: Yes */

/* Rotates a tetromino on it's origin point counterclockwise 90 degrees if the desination coordinates are valid */
int rotateTetrominoCCWise(Tetromino *t, Play_area *p); /* Tested: Yes */

/* Shifts a specific Tetromino block to the next X and Y coordinates */
void shift(Tetromino *t, int nextX, int nextY, int i); /* Tested: Yes */

/* Wrapper function that initializes the spawn coordinates for tetromino passed in to the char type */
void initializeAllTetrominoSpawn(Tetromino *t, char type); /* Tested: Yes */
...
/* Checks if the coordinates entered are occupied, empty, or out of bounds*/
...
void initPlayArea(Game_Model *model); /* Tested: Yes */

/* Tests to see if the specified row is full. Rerturns True or False. */
int isRowFull(Game_Model *model, int yRow); /* Tested: Yes */

/* Clears an entire specified Play_area row (y-coordinate) of occupied blocks. */
void clearRow(Game_Model *model, int yRow); /* Tested: Yes */
...
void initScoreDisplay(Game_Model *model); /* Tested: Yes */

/* Initializes the top score label to the default screen coordinates with default value "TOP SCORE" */
void initTopScoreLabel(Game_Model *model); /* Tested: Yes */

/* Initializes the current score label to the default screen coordinates with default value "SCORE" */
void initCurrentScoreLabel(Game_Model *model); /* Tested: Yes */

/* Initializes the level label to the default screen coordinates with default value "LEVEL" */
void initLevelLabel(Game_Model *model); /* Tested: Yes */

/* Initializes the top score string value to the screen coordinates and starting value "000000" */
void initTopScoreString(Game_Model *model); /* Tested: Yes */

/* Initializes the current score string value to the screen coordinates and starting value "000000" */
void initCurrentScoreString(Game_Model *model); /* Tested: Yes */

/* Initializes the current score string value to the screen coordinates and starting value "00" */
void initLevelString(Game_Model *model); /* Tested: Yes */

/*
    Sets all game score and level number string values to reflect
    the Game_Model Score_display object values.
*/
void setScoresAndLevel(Game_Model *model); /* Tested: Yes */

/* 
    Increments the game score stored in a Score_display object.
    Increment amount is determined by the level, where the level
    value is a "multiplier" to the default value.
 */
void incrementScore(Game_Model *model); /* Tested: Yes */

/* Sets the Score Display object top score to a predesired value. Largest value that will display is 999,999 */
void setTopScore(Game_Model *model, unsigned long top_score); /* Tested: Yes */

/* Increments the game level stored in a Score_display object by 1. Limited to max level of 10. */
void incrementLevel(Game_Model *model); /* Tested: Yes */

/*Adjusts the top score to be equal to the current score if the current score exceeds the top score value. */
void updateTopScore(Game_Model *model); /* Tested: Yes */
...
/* (W.I.P) Initializes the next tetromino display coordinates and block offset values */
void initNextTetromDisplay(Game_Model *model); /* Tested: Yes */
