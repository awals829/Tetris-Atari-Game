...
void modelTetrominoCopyToTemp(Game_Model *model) {
    int i;
    for(i = 0; i < TETROMINO_SIZE; i++) {
        model->temp.blocks[i].arenaX = model->t.blocks[i].arenaX;
        model->temp.blocks[i].screenX = model->t.blocks[i].screenX;
        model->temp.blocks[i].arenaY = model->t.blocks[i].arenaY;
        model->temp.blocks[i].screenY = model->t.blocks[i].screenY;
        model->temp.blocks[i].is_occupied = model->t.blocks[i].is_occupied;
        model->temp.blocks[i].is_tetromino_piece = model->t.blocks[i].is_tetromino_piece;
    }
    model->temp.tetromino_type = model->t.tetromino_type;
}
 ...
 */
char randomize(Tetromino *t, long seed) {
    char blocks[7] = {'I', 'O', 'T', 'S', 'Z', 'L', 'J'};
    long index = (seed + t->blocks[0].arenaX 
                + t->blocks[1].arenaY
                + t->blocks[2].arenaX
                + t->blocks[3].arenaY) & 6;
...
void initPlayArea(Play_area *p) {
    ...
    p->gravity = DEFAULT_PLAY_AREA_GRAVITY;
...
/*
void clearFilledLines(Play_area *p) {
    int x, y, isRowFilled;
    for(y = PLAY_AREA_MAX_Y; y >= 0; y--) {
        isRowFilled = TRUE;
        ...
        if(isRowFilled) {
            for(x = 0; x <= PLAY_AREA_MAX_X; x++) {
                p->arena[x][y].is_occupied = 0;
                p->arena[x][y].is_tetromino_piece = 0;
            }
            dropBlocksAbove(p, y);
            y++;
        }
...
void dropBlocksAbove(Play_area *p, int clearedRow) {
    for (int y = clearedRow; y > 0; y--) {
        for (int x = 0; x <= PLAY_AREA_MAX_X; x++) {
            p->arena[x][y].is_occupied = p->arena[x][y - 1].is_occupied;
            p->arena[x][y].is_tetromino_piece = p->arena[x][y - 1].is_tetromino_piece;
        }
    }
    // Clear the top row as it has now been shifted down
    for (int x = 0; x <= PLAY_AREA_MAX_X; x++) {
        p->arena[x][0].is_occupied = 0;
        p->arena[x][0].is_tetromino_piece = 0;
    }
}
*/
 ...
 * 
 * Purpose: Initializes the display area for the next tetromino, setting up its position, size, and appearance.
 * Input: ntd - A pointer to the Next_tetrom_display structure to be initialized.
 * Output: None.
...
void initNextTetromDisplay(Next_tetrom_display *ntd) {
    ntd->screenX = NEXT_TETROM_DISPLAY_SCREENX;
    ntd->screenY = NEXT_TETROM_DISPLAY_SCREENY;
    ntd->width = NEXT_TETROM_DISPLAY_WIDTH_PX;
    ntd->height = NEXT_TETROM_DISPLAY_HEIGHT_PX;
}