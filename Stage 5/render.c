...
void renderAllObjects(const Game_Model *model, void *base) {
    renderPlayArea(&model->p, (UINT8 *) base);
    renderBlocks(&model->p, (UINT16 *) base);
    clearTetrominoRender(&model->t, (UINT16 *) base);
    renderTetromino(&model->t, (UINT16 *) base);
    renderScoreDisplay(&model->sd, (UINT8 *) base);
    renderScoreDisplayValues(model, (UINT8 *) base);
    renderNextTetrominoArea(&model->ntd, (UINT8 *) base);
}
 ...
 */
void renderPlayArea(const Play_area *p, UINT8 *base) {
 ...
 */
void renderNextTetrominoArea(const Next_tetrom_display *ntd, UINT8 *base) {
 ...
 */
void renderScoreDisplay(const Score_display *sd, UINT8 *base) {
...
void renderScoreDisplayValues(const Game_Model *model, UINT8 *base) {
    int i, x;
    renderTopScoreLabel(&model->tsl, base);
    renderTopScoreNumString(&model->tss, base);
    renderCurrentScoreLabel(&model->csl, base);
    renderCurrentScoreNumString(&model->css, base);
    renderLevelLabel(&model->lvlL, base);
    renderLevelNumString(&model->lvlS, base);
}
